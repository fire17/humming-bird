...
from hummingbird_game import (_glyph_foreground_mask, FRAME_COUNT, SPEED_STEPS,
                              TRAIL_COLORS, RETICLE_CELL, perch_cell)
...
def main():
    ...
    leaf_index = len(sprites)-1
    dim_leaf_index = len(sprites)
    sprites.append({point: perch_cell(cell, False) for point, cell in leaf.items()})
    effects = {}
    for cell in [*(Char(data='·', fg=color) for color in TRAIL_COLORS), RETICLE_CELL]:
        effects[cell.data + ':' + cell.fg] = len(sprites)
        sprites.append({(0, 0): cell})
    ...
    for index, sprite in enumerate(sprites):
        ...
        for (x, y), cell in sprite.items():
            ...
            # Text effects need actual thin glyph coverage, not the compositor's
            # conservative full-cell fallback. Leave every approved art tile intact.
            if index in effects.values():
                if cell.data == '·':
                    mask = tuple(2 <= x < 6 and 3 <= y < 5 for y in range(8) for x in range(8))
                elif cell.data == '+':
                    mask = tuple((1 <= x < 7 and 3 <= y < 5) or (3 <= x < 5 and 1 <= y < 7)
    ...
    (out / "assets.json").write_text(json.dumps(dict(
        build=build, hashes=hashes, frames=FRAME_COUNT, hearts=len(hearts), leaf=leaf_index,
        dimLeaf=dim_leaf_index, effects=effects,
